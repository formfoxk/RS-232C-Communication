...
IMPLEMENT_DYNCREATE(Comm, CCmdTarget)  
  
Comm::Comm(CString port,CString baudrate,CString parity,CString databit,CString stopbit)  
{  
    m_sComPort = port;  
...
BEGIN_MESSAGE_MAP(Comm, CCmdTarget)  
    //{{AFX_MSG_MAP(Comm)  
    ...
    while (Comm->m_bIsOpenned) {  
        ...
        if ((EvtMask & EV_RXCHAR) && ComStat.cbInQue) {  
            ...
            do {  
                ...
                insize += Length;  
            } while ((Length!=0) && (insize<size)); ClearCommError(Comm->m_hComDev, &ErrorFlags, &ComStat);
...
BOOL Comm::Send(LPCTSTR outbuf, int len)  
{  
    BOOL    bRet=TRUE;  
    DWORD       ErrorFlags;  
    COMSTAT     ComStat;  
...
int Comm::Receive(LPSTR inbuf, int len)  
{  
